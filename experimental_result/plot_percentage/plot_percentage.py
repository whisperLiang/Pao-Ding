...
#条形图的绘制
labels = ['Pao-Ding', 'ResMap', 'PDD', 'AutoDiCE']
...
ax_trans1=[0.252, 0.395, 1.186, 1.188]
ax_inference1=[1.431, 1.255, 0.862, 1.051]
ax_inference1=[round(i,3) for i in ax_inference1]
ax_trans2=[1.416, 1.822, 1.912, 2.105]
ax_inference2=[0.722, 0.582, 0.561, 0.566]


width = 0.35  # 条形图的宽度
fig,ax = plt.subplots(figsize=(6,5))
plt.tick_params(labelsize=15)
re1=ax.bar(labels, ax_trans1,width,color='lightgreen', hatch = '///',label=r'Master$\rightarrow$Work 0 ',zorder=10)
re2=ax.bar(labels,ax_inference1,width,bottom=ax_trans1,color='lightblue', hatch =  '*',label='Inference in Work 0',zorder=10)
re3=ax.bar(labels,ax_trans2,width,bottom=[ax_trans1[i]+ax_inference1[i] for i in range(4)],color='lightgreen',hatch = '..',label=r'Work 0$\rightarrow$Work 1 ',zorder=10)
re4=ax.bar(labels,ax_inference2,width,bottom=[ax_trans1[i]+ax_inference1[i]+ax_trans2[i] for i in range(4)],color='lightblue',hatch = '++',label='Inference in Work 1',zorder=10)
...
plt.savefig('AlexNet-baseline.png', dpi=300, bbox_inches='tight')


labels = ['Pao-Ding', 'PDD', 'AutoDiCE']
vgg_trans1=[0.364, 1.178, 1.179]
vgg_inference1=[3.965, 3.246, 3.373]
vgg_trans2=[2.765, 4.606, 4.688]
vgg_inference2=[4.687, 4.328, 4.386]
fig,ax = plt.subplots(figsize=(6,5))
plt.tick_params(labelsize=15)
re1=ax.bar(labels, vgg_trans1,width,color='lightgreen', hatch = '///',label=r'Master$\rightarrow$Work 0 ',zorder=10)
re2=ax.bar(labels,vgg_inference1,width,bottom=vgg_trans1,color='lightblue', hatch =  '*',label='Inference in Work 0',zorder=10)
re3=ax.bar(labels,vgg_trans2,width,bottom=[vgg_trans1[i]+vgg_inference1[i] for i in range(3)],color='lightgreen',hatch = '..',label=r'Work 0$\rightarrow$Work 1 ',zorder=10)
re4=ax.bar(labels,vgg_inference2,width,bottom=[vgg_trans1[i]+vgg_inference1[i]+vgg_trans2[i] for i in range(3)],color='lightblue',hatch = '++',label='Inference in Work 1',zorder=10)
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
ax.set_ylabel('Scores')
ax.set_ylabel('Processing Time (s)',fontsize=15)
...
plt.grid(axis="y",linestyle='-.',zorder=0)
x=[0.325,1.325,2.325,3.325]
y11=[i/2+0.1 for i in vgg_trans1]
y21=[vgg_trans1[i]+vgg_inference1[i]/2+1 for i in range(3)]
y31=[vgg_trans1[i]+vgg_inference1[i]+vgg_trans2[i]/2+1 for i in range(3)]
y41=[vgg_trans1[i]+vgg_inference1[i]+vgg_trans2[i]+vgg_inference2[i]/2+1 for i in range(3)]
y1=[y1,y2,y3,y4]
for i in range(3):
...
plt.savefig('InceptionV3-baseline.png', dpi=300, bbox_inches='tight')